---
title: 'Updater'
description: 'Update strategy, updater configuration, and file backups.'
---
...
> **Warning:** **Note:** At the moment the automatic file replacement is only available on Paper. On Velocity an update folder will be created within the plugin directory, you have to move the files yourself.

> **Warning:** **Note:** Velocity is currently working on the update folder feature.
...
## File Backups
If the plugin updates itself, there is a possibility that the config or language files will be updated. 
To prevent data loss, the plugin will create a backup of the files before updating them. The backups are stored in the `backups` folder in the plugin directory.
There are several things you have to know about the backups and updated files:
* Once the maximum number of backups is reached, the oldest backup will be deleted. You can set the maximum number of backups in the `config.yml`.
* You can disable the backup limit by setting the `maxBackups` to `0`.
